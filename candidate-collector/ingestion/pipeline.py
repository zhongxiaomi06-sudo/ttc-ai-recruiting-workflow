...
def local_duplicate_exists(record: CandidateRecord) -> dict[str, Any] | None:
    ...
    with closing(_db_conn()) as conn:
        if record.attachment_sha256:
            row = conn.execute(
                "SELECT * FROM ingestion_log WHERE attachment_sha256 = ? LIMIT 1",
        ...
        if record.phone:
            row = conn.execute(
                "SELECT * FROM ingestion_log WHERE phone = ? LIMIT 1",
                (record.phone,),
            ).fetchone()
            if row:
                return dict(row)
        if record.name and record.current_company:
            row = conn.execute(
                "SELECT * FROM ingestion_log WHERE name = ? AND current_company = ? LIMIT 1",
                (record.name, record.current_company),
            ).fetchone()
            if row:
                return dict(row)
    return None
...
def ingest_text(
    ...
    *,
    ...
    """Ingest raw resume text (e.g. from browser extension)."""
    init_ingestion_tables()
